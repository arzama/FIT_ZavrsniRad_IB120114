...
namespace PEPmodnaKompanija_API.Util
{
    public class Preporuka
    {
        ...
        public List<usp_Proizvodi_SelectById_Result> GetSlicneProizvode(int proizvodId)
        {
            UcitajProizvode(proizvodId);
            List<Ocjene> ocjene = db.Ocjene.Where(x => x.ProizvodID == proizvodId).OrderBy(x => x.KupacID).ToList();

            List<Ocjene> zajednickeOcjene1 = new List<Ocjene>();    
            List<Ocjene> zajednickeOcjene2 = new List<Ocjene>();  


            List<usp_Proizvodi_SelectById_Result> preporuceno = new List<usp_Proizvodi_SelectById_Result>();
            ...
            foreach (var item in proizvodi)
            {
                ...
                if (slicnost > 0.6) 
                    preporuceno.Add(db.usp_Proizvodi_SelectById(item.Key).FirstOrDefault());
        ...
        {
            List<Proizvodi> aktivniProizvodi = db.Proizvodi.Where(x => x.Status == true && x.ProizvodID != proizvodId).ToList();
